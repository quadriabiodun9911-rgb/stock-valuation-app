...
@router.post("/posts")
async def create_post(body: CreatePost, user_id: int = Depends(get_user_id)):
    post = db.create_post(user_id, body.content, body.symbol)
    return post


@router.get("/feed")
async def get_feed(limit: int = 50, offset: int = 0, user_id: int = Depends(get_user_id)):
...
async def toggle_like(post_id: int, user_id: int = Depends(get_user_id)):
...
async def add_comment(post_id: int, body: CreateComment, user_id: int = Depends(get_user_id)):
...
@router.get("/friends")
async def get_friends(user_id: int = Depends(get_user_id)):
    return {"friends": db.get_friends(user_id)}


@router.get("/friends/requests")
async def get_friend_requests(user_id: int = Depends(get_user_id)):
    return {"requests": db.get_friend_requests(user_id)}


@router.post("/friends/{addressee_id}")
async def send_friend_request(addressee_id: int, user_id: int = Depends(get_user_id)):
...
async def respond_to_request(request_id: int, body: FriendAction, user_id: int = Depends(get_user_id)):
...
async def search_users(q: str, user_id: int = Depends(get_user_id)):
    if len(q) < 2:
...
async def get_conversations(user_id: int = Depends(get_user_id)):
...
async def send_message(receiver_id: int, body: SendMessage, user_id: int = Depends(get_user_id)):
